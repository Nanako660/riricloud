...
- 节点与线路架构重构：Node 仅保留底座机器与 Agent/内核状态，新增 `isLocal`；用户订阅改由独立 Line 承载地址/端口、SNI/Host、倍率、标签、等级和启停状态。
...
- 线路对外覆盖新增默认关闭的 `endpointOverrideEnabled` 开关：关闭时复用入口节点/目标入站的地址、端口、SNI 与 Host，保留已填写的覆盖值供重新启用。
- 改进本地一键联调脚本：主控进程异常退出时立即显示最近 40 行服务端日志，避免启动失败时长时间无反馈。
- 优化新建入站与中继入口：默认监听 IPv4 通配地址 `0.0.0.0`，未指定端口时由服务端随机分配 `20000~29999` 的五位端口；编辑既有资源时保持原端口不变。
- 优化入站弹窗：端口改为直接预填五位随机端口并简化标签；无协议专属字段时隐藏空白的专属参数卡片。
...
- 修复本机 VLESS 入站错误组合 `xtls-rprx-vision` + 明文 TLS 配置导致客户端握手超时；服务端现在会自动清除明文 VLESS 的 flow，seed 默认生成有效 Reality 配置，并迁移修复存量记录。
- 节点删除保护：主控本机 `Master-Local` 节点不再允许删除，管理端隐藏对应删除入口，服务端接口统一返回 `409`。
- 修复线路管理接口脱敏 `targetInbound` 时遗漏目标节点摘要导致页面读取 `targetInbound.node.name` 崩溃；本地联调脚本默认复用 seed 预置的 `Master-Local`，避免 Agent 连接到临时节点后面板本机节点仍显示离线。
