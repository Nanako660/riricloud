...
### Fixed
- **套餐卡片与管理端实机预览浅色模式 (Light Mode) 视觉适配**：修复浅色主题下晶体按钮文本颜色过浅（如 `text-amber-200`）导致不可读、卡片白边框在浅色背景下溶解消失、顶部倒角折射微线与特性微标记图标浅色泛白等对比度缺陷；建立明暗双模动态色阶体系（深色高光、浅色高对比深色调），卡片内衬流体极光光池在浅色模式下自动收敛为 35% 柔和水彩光晕，实现明暗双模式下统一高雅的微透磨砂与晶体质感。
