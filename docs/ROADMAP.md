...
- [x] ⭐ 实现 JWT 认证模块（登录、当前用户信息、角色守卫）。（注册端点待注册开关落地）
- [ ] 实现用户管理与配额控制模块。（⭐ 已实现用户面板只读端点，管理操作待补）
- [x] ⭐ 实现节点管理模块（节点创建/列表、AgentToken 派发、Reality 参数生成、热重载指令）。（编辑/删除待补）
...
- [ ] 开发用户端界面：
  - [x] ⭐ 用户登录页。（注册页待注册开关落地）
  - [x] ⭐ 个人仪表盘（剩余流量进度条、账户有效期、一键复制/导入订阅）。
  - [x] ⭐ 可用节点列表与延迟状态。（延迟展示待内核接入）
- [ ] 开发管理员控制台：
  - [ ] 用户管理（配额修改、封禁/解封、角色设置）。
  - [x] ⭐ 节点管理（节点添加、AgentToken 与一键安装命令展示、实时 CPU/内存/带宽负载监控）。（编辑/删除待补）
  - [ ] 系统设置（注册开关、全局默认配额）。
