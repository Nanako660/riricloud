...
> **当前进度**：Phase 1 已完成；v0.3.0 订阅架构重构已完成并打通套餐、唯一订阅、模板编译、节点授权、远程升级与网络探针闭环。下方标注 ⭐ 的条目为已实现部分；按用户流量统计仍受 sing-box 上游能力限制暂缓（见 Phase 3 遥测条目）。视觉验证按项目约束仅在 Antigravity 环境按需执行，不接入常规门禁。
...
- [x] Server：TypeScript、ESLint、Jest（12 suites / 108 tests）通过。
...
- [ ] 视觉截图回归：按 `docs/VISUAL_VERIFICATION.md` 仅在 Antigravity 环境、收到明确视觉验证请求后执行。
