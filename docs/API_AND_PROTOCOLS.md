...
### 1.1 认证模块 (`/auth`)
- `POST /auth/register`：用户注册（若系统开启注册开关）。
...
- `POST /user/reset-sub`：重置用户的 `subscriptionToken`（防止订阅泄漏）。

### 1.3 管理员模块 (`/admin`)
- `GET /admin/users`：分页查询用户列表，支持搜索。
- `POST /admin/users`：创建新用户。
- `PATCH /admin/users/:id`：修改用户配额、到期时间、角色与激活状态。
- `DELETE /admin/users/:id`：删除用户。
- `GET /admin/nodes`：获取所有节点详情（包含 AgentToken 与遥测状态）。
- `POST /admin/nodes`：创建新节点（生成一键安装脚本与 Token）。
- `PATCH /admin/nodes/:id`：修改节点参数（名称、IP、端口、协议参数）。
...
### 3.1 客户端请求头自动识别与参数适配
- 若请求参数包含 `?type=clash` 或 User-Agent 包含 `Clash` / `meta` / `Mihomo`，输出 **Clash Meta YAML**。
- 若请求参数包含 `?type=sing-box` 或 User-Agent 包含 `sing-box`，输出 **Sing-box Client JSON**。
