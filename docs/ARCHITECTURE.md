...
- **Web UI (`apps/web`)**：为用户和管理员提供现代化的 Web 控制界面。包括用户注册登录、流量仪表盘、我的订阅与可用线路概览、通用订阅导出，以及管理员的用户管理、节点纳管、线路拓扑配置、配置下发和系统状态监控。
...
- **配置推送与在线门禁**：保存线路后复用 250ms 防抖，自动为承担入口或出口角色的在线节点重新编译并下发 `config_sync`。中继线路在套餐订阅中要求入口节点与出口节点**同时在线**才会向客户端呈现并允许连通。
