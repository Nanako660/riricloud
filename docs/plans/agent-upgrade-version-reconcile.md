...
- [ ] 复现用例：升级成功后心跳持续上报旧版本 → 5 分钟内主控产生 WARN 系统日志，详情页出现警示横幅
- [ ] 心跳上报目标版本 → INFO 确认日志、条目清除、横幅消失、画像显示新版本
- [ ] 重连注册不再重发 60 秒内已派发任务（单测覆盖）
- [ ] restart 决策矩阵单测：running→服务重启；未安装/出错→自拉起；服务重启失败→降级自拉起；双失败→返回错误
- [ ] `pnpm gate`（version/docs/server/web/agent）全绿
- [ ] 真机验证记录：Linux systemd 安装模式 / 免安装前台 / 容器（无 systemd 自动降级自拉起）
