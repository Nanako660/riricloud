...
| **`UI-20`** | 用户订阅 | 套餐市场 | `/market` | `apps/web/src/pages/user/market/index.tsx`, `components/market-plan-card.tsx` | 快捷周期筛选 Tab（全部/月付/季/半年/年付）带计数、现代极客流体极光卡片 MarketPlanCard、北欧极光流体光池 (Nordic Aurora Orbs) 缓动漂浮、晶体微光边缘漫射折射 (Crystal Sheen)、顶部 1px 倒角微光折射线 (Chamfered Edge Light)、鼠标光斑追踪 Spotlight、按钮金属微光扫光 Shimmer、半透微折射晶体按钮 (Smoked Glass Button)、微透磨砂价格面板、折扣文案与多样化角标、特性清单微标记图标解析渲染（[zap], [rocket], [crown], [shield], [sparkles], [star], !）、当前在用套餐与余额横幅、低价套餐禁止直接降级、订购/升配二次确认弹窗（含卡密快速兑换）、窄屏单列自适应 |
| **`UI-21`** | 用户订阅 | 我的订阅综合控制台 | `/subscription` | `apps/web/src/pages/user/subscription/index.tsx` | 公告下方按状态渲染订阅主体或开通引导；有订阅时展示流量进度、状态 Badge、流量重置策略与统一时区下次重置时间、标准/伪静态 Token URL 复制/重置、取消保留权益提示、可用线路紧凑列表（名称/协议/统一时区延迟 Chip/在线状态/倍率）；强制邮箱验证开启且未验证时隐藏顶部 PageHeader 并呈现视觉居中的服务暂不可用全屏卡片（彻底隐藏公告、套餐卡片、订阅链接、节点列表及使用指引，仅展示居中偏上的盾牌图标卡片、统一暂不可用文案与原地即时验证弹窗）；无订阅时展示前往套餐市场按钮；小型确认弹窗在移动端保持圆角、左右留白、居中紧凑且 Footer 横向排列 |
