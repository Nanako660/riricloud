...
| **`UI-17`** | 套餐管理 | 套餐创建/编辑弹窗与实机即时预览 | `/admin/plans`（点击“新建套餐/编辑”） | `apps/web/src/pages/admin/plans/components/plan-form-dialog.tsx` | 配额/期限数值输入、流量重置策略 Select、匹配模式 Select、模板选择、公开 Switch、角标文本 badgeText、主推推荐套餐 isFeatured 开关、自定义权益清单 features 多行文本与快捷微标记预设；视觉与动效配置（7 款质感主题色选择、12 款 Lucide 专业矢量图标网格、流体极光与晶体微光漫射动效模式及光色选择、原价划线与折扣文案、按钮扫光开关、同步特效至「我的订阅」syncToSubscription 开关）；右侧所见即所得实机卡片预览（Live Preview）实时联动 135° 对角全色彩深度填充渐变（bg-gradient-to-br）、Nordic Aurora 与微透磨砂价格容器、移动端自适应滚动 |
...
| **`UI-20`** | 用户订阅 | 套餐市场 | `/market` | `apps/web/src/pages/user/market/index.tsx`, `components/market-plan-card.tsx` | 快捷周期筛选 Tab（全部/月付/季/半年/年付）带计数、现代极客流体极光卡片 MarketPlanCard、明暗双模 135° 对角线左上至右下全色彩深度填充渐变（bg-gradient-to-br，浅色温润柔和与深色星云宝石，彻底去除中性底色截断）、北欧极光流体光池 (Nordic Aurora Orbs) 缓动漂浮、晶体微光边缘漫射折射 (Crystal Sheen)、顶部 1px 倒角微光折射线 (Chamfered Edge Light)、鼠标光斑追踪 Spotlight、按钮金属微光扫光 Shimmer、半透微折射晶体按钮 (Smoked Glass Button)、微透磨砂价格面板（backdrop-blur-sm bg-white/50 或 dark:bg-black/30 透出底层对角全渐变与流光）、折扣文案与多样化角标、特性清单微标记图标解析渲染（[zap], [rocket], [crown], [shield], [sparkles], [star], !）、当前在用套餐与余额横幅、低价套餐按钮清晰提示「暂不支持降级」且无底部冗余换行文案、订购/升配二次确认弹窗（含卡密快速兑换）、窄屏单列自适应 |
| **`UI-21`** | 用户订阅 | 我的订阅综合控制台 | `/subscription` | `apps/web/src/pages/user/subscription/index.tsx` | 公告下方按状态渲染订阅主体或开通引导；当前订阅主卡片支持受控同步当前套餐同款 135° 对角全色彩深度填充渐变底色（bg-gradient-to-br）、流体极光与晶体漫射折射微边框动效（受全局与套餐级双层开关 subscriptionEffectsSyncEnabled 控制；关闭时干净降级为极简原生无动效卡片），内部无论动效开启与否均保持完全原生的交互与视觉规范（标题区原生利落文字排版、状态与推荐 Badge 原生样式、操作按钮统一维持原生 outline 风格且无侵入性渐变玻璃；动效开启时 4 块指标卡自适应微透磨砂玻璃 backdrop-blur-sm 透显对角渐变底色，动效关闭时恢复原生 bg-muted/20 清爽排版）；流量进度、状态 Badge、流量重置策略与统一时区下次重置时间、标准/伪静态 Token URL 复制/重置、取消保留权益提示、可用线路紧凑列表（名称/协议/统一时区延迟 Chip/在线状态/倍率）；强制邮箱验证开启且未验证时隐藏顶部 PageHeader 并呈现视觉居中的服务暂不可用全屏卡片（彻底隐藏公告、套餐卡片、订阅链接、节点列表及使用指引，仅展示居中偏上的盾牌图标卡片、统一暂不可用文案与原地即时验证弹窗）；无订阅时展示前往套餐市场按钮；小型确认弹窗在移动端保持圆角、左右留白、居中紧凑且 Footer 横向排列 |
