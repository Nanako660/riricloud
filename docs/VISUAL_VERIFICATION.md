...
| **`UI-19`** | 模板管理 | 订阅模板编辑弹窗与预览 | `/admin/templates`（点击“新建模板/编辑”） | `apps/web/src/pages/admin/templates/components/template-form-dialog.tsx`, `template-groups-editor.tsx`, `template-rules-editor.tsx`, `template-dns-editor.tsx`, `template-override-editor.tsx`, `template-preview-drawer.tsx` | 六 Tab 独立响应式切换、DNS 直连/代理徽章列表与常用预设快捷添加、客户端高级覆写全宽编辑器与 Clash/Sing-box 二级切换、常用配置片段智能 deepMerge 注入、实时语法状态检测、可视化卡片/CodeMirror 双模、编辑器跟随明暗主题、Clash/Sing-box 预览切换、桌面/移动端滚动与双主题 |
