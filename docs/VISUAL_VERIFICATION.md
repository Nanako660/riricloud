...
| **`UI-30`** | 卡密管理 | 分类、奖励/限额、生成及删除/作废交互 | `/admin/redeem-codes` | `apps/web/src/pages/admin/redeem-codes/**` | 四态统计（数量 + 金额）、分类/状态/已删除筛选、防抖搜索、分类奖励 BALANCE/PLAN 与购买身份累计限额配置、按分类生成并展示奖励快照说明、标签管理、卡密默认掩码与明文切换/复制、CSV/TXT 当前筛选导出、未使用卡显式作废、批量/单张软删除确认与已删除视图恢复、过期清理只软删除、兑换人与兑换时间、有效期状态、移动端表格局部滚动及分类/表单弹窗滚动 |
