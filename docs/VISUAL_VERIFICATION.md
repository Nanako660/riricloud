...
| **`UI-06`** | 节点管理 | 添加节点弹窗 | `/admin/nodes`（点击“添加节点”） | `apps/web/src/pages/admin/nodes/components/node-form-dialog.tsx` | Dialog 居中、网络可达性单选（公网 VPS / NAT 落地）、服务器地址与名称输入框（NAT 模式下地址设为选填并自动提供 127.0.0.1 兜底与反向隧道接入提示）、通信协议选择、创建成功后展示 AgentToken，并提供原生 CLI 与 Docker 容器启动双 Tab 命令切换与复制 |
...
| **`UI-09`** | 节点详情 | 基础信息与遥测 Tab 及安装弹窗 | `/admin/nodes/:id` (Tab 2) | `apps/web/src/pages/admin/nodes/detail.tsx`, `apps/web/src/pages/admin/nodes/components/rotate-token-dialog.tsx` | 节点基础信息编辑、远程节点 AgentToken 轮换入口、本机节点轮换禁用说明、轮换二次确认弹窗、一次性新 Token 与安装命令弹窗（支持原生 CLI 与 Docker 容器双 Tab 及 WS/HTTP 切换）、通信模式、Agent/系统架构/内核版本画像、遥测与内核状态、统一时区上报时间 |
...
| **`UI-31`** | 资源管理 | 资源管理与资源详情 | `/admin/binaries` | `apps/web/src/pages/admin/binaries/**` | 服务端分页/搜索（版本号或备注，400ms 防抖）与类型/平台/状态组合筛选；行多选与批量工具栏（启用/停用/归档/删除，危险动作二次确认）；数据表格紧凑布局、默认/启停/归档状态 Badge、平台架构徽标聚合（超过 3 个折叠 +N 并带完整平台与体积 Tooltip）、总体积与引用分发次数、分页底栏；操作列详情/启停外露与编辑备注兼容性/设为默认/归档恢复/删除下拉收纳；本地上传（选文件后浏览器自动计算 SHA-256 并展示计算态）与远程导入表单（react-hook-form + zod 字段级错误提示）；资源详情弹窗（备注、兼容性约束徽标、完整 SHA-256、分发记录分页列表与状态筛选、失败任务就地重试）；操作审计弹窗（分页、动作筛选、操作者展示）；明暗主题与移动端局部滚动 |
