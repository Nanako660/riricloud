...
| **`UI-21`** | 用户订阅 | 我的订阅综合控制台 | `/subscription` | `apps/web/src/pages/user/subscription/index.tsx` | 公告下方按状态渲染订阅主体或开通引导；当前订阅主卡片融入当前套餐同款主题色渐变浸润底色、流体极光与晶体漫射折射微边框动效，内部保持完全原生的交互与视觉规范（标题区原生利落文字排版、状态与推荐 Badge 原生样式、操作按钮统一维持原生 outline 风格且无侵入性渐变玻璃、4 块指标卡保持原生 bg-muted/20 清爽排版）；流量进度、状态 Badge、流量重置策略与统一时区下次重置时间、标准/伪静态 Token URL 复制/重置、取消保留权益提示、可用线路紧凑列表（名称/协议/统一时区延迟 Chip/在线状态/倍率）；强制邮箱验证开启且未验证时隐藏顶部 PageHeader 并呈现视觉居中的服务暂不可用全屏卡片（彻底隐藏公告、套餐卡片、订阅链接、节点列表及使用指引，仅展示居中偏上的盾牌图标卡片、统一暂不可用文案与原地即时验证弹窗）；无订阅时展示前往套餐市场按钮；小型确认弹窗在移动端保持圆角、左右留白、居中紧凑且 Footer 横向排列 |
