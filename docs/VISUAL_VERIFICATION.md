...
| **`UI-19`** | 模板管理 | 订阅模板编辑弹窗与预览 | `/admin/templates`（点击“新建模板/编辑”） | `apps/web/src/pages/admin/templates/components/template-form-dialog.tsx`, `template-groups-editor.tsx`, `template-rules-editor.tsx`, `template-dns-editor.tsx`, `template-override-editor.tsx`, `template-source-editor.tsx`, `template-preview-drawer.tsx` | 六 Tab 独立响应式切换（基本信息、策略组设计、分流规则、DNS 设置、客户端高级覆写、源文件编辑）、策略组与规则预设沉浸式顶栏、DNS 直连/代理徽章列表与主流预设、客户端高级覆写全宽编辑器与 Clash/Sing-box 智能 deepMerge 注入、整模板 JSON 源文件双向编辑与错误隔离守卫、Sing-box 内核真实 check 校验与诊断日志抽屉、编辑器跟随明暗主题、桌面/移动端滚动与双主题 |
