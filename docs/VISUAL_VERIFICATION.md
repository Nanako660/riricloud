...
>
> **2026-09-26 增量台账（资源管理重构与交互精简）**：统一侧栏与顶栏标题为「资源管理」，移除顶栏系统日志按钮与顶部冗余体积统计卡片；将「从 GitHub Release 获取资源」与「URL 导入 Agent 资源」合并为「远程导入」双标签页弹窗（默认 GitHub Release，仓库地址 `githubRepoUrl` 统一收敛至系统设置并在构建期默认内嵌项目公开仓库地址）；状态列改为交互式 `Switch` 开关（仅保留启用/停用状态，移除操作列重复启停与归档按钮）；资源详情仅保留「总体积」展示。受影响索引为 `UI-14`、`UI-31`、`UI-32`。当前实现未将视觉检查接入 CI 或 Git Hook。
