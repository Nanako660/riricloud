...
| **`UI-31`** | 资源管理 | 资源管理与资源详情 | `/admin/binaries` | `apps/web/src/pages/admin/binaries/**` | 版本号关键词即时搜索与类型/平台/状态组合筛选；数据表格紧凑布局、默认/启停/归档状态 Badge、平台架构徽标聚合（超过 3 个折叠 +N 并带完整平台与体积 Tooltip）、总体积与引用分发次数、操作列详情/启停外露与设为默认/归档下拉收纳；本地上传、远程导入、资源详情抽屉（完整 SHA-256 与分发历史）；明暗主题与移动端局部滚动 |
