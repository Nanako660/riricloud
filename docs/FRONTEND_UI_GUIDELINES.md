...
| 卡密管理 | `/admin/redeem-codes` | 按分类与标签管理余额/套餐奖励、购买身份累计限额、按分类批量生成与奖励快照；四态统计、分类/状态/已删除筛选、卡密掩码/明文切换、CSV/TXT 导出、行选择软删除/批量作废、恢复与过期清理 | 余额以元输入、服务端按分保存；分类默认每购买身份限兑 1 次，支持正整数或不限；列表默认掩码显示（保留前缀段与末 4 位）；生成结果在弹窗集中复制。软删除/作废/清理均使用 `AlertDialog` 确认；软删除不作废卡密，已删除视图可逐条恢复 |
...
2. **严禁硬编码文本**：全站用户可见文案（含表单占位提示 placeholder、错误校验提示、Toast 反馈、二次确认弹窗正文、Tooltip 说明）必须统一收敛至多语言字典，严禁在 TSX 中直接裸写字符串。
3. **带变量插值规范**：插值变量采用 `{{variable}}` 语法并在 TS 泛型或字典中显式声明类型；传递可选值（如实体名称）时必须提供安全空字符串兜底（如 `{ name: entity?.name ?? '' }`）。
