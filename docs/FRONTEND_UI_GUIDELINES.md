...
| 订阅模板 | `/admin/templates` | 模板卡片、复制/预览操作、策略组与规则集双模编辑、语义化 DNS、Clash/Sing-box 覆写 | 内嵌默认模板显示“内嵌”标记；复制与快速预览使用图标+文字按钮；编辑弹窗使用五个 Tab，结构化卡片与 CodeMirror 源码模式双向同步；删除使用 AlertDialog；JSON/YAML 由 Zod 与服务端双重校验 |
...
新增页面、弹窗和节点升级入口的编号、源码路径、明暗主题检查点统一登记在 [docs/VISUAL_VERIFICATION.md](VISUAL_VERIFICATION.md) 的 UI-01 至 UI-32。视觉验证仍按需执行，不能接入 CI 或 Git Hook。
