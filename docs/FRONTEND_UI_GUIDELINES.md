...
| 我的订阅 | `/subscription` | 系统公告、当前套餐完整画像、流量进度、到期时间、Token 链接、可用线路紧凑列表、客户端使用指引 | 重置 Token 与取消订阅必须经 AlertDialog；无订阅时展示前往套餐市场的开通引导；消除冗余复制按钮，唯一使用输入框内联 CopyButton；提供前往套餐市场的升配入口；线路仅显示名称、协议类型、在线状态与倍率 |
...
- 编辑用户弹窗使用 `Tabs` 分为「账号安全」和「订阅管理」：前者管理角色、启用/封禁与密码重置，后者管理套餐、状态、配额、已用流量、延期和 Token 重置。
...
- 小型确认类 `AlertDialog` 保持默认居中弹窗尺寸；移动端 Footer 覆盖为紧凑横向按钮，按钮使用自然宽度并保持确认操作靠右，取消按钮不得保留纵向堆叠间距。
...
新增页面、弹窗和节点升级入口的编号、源码路径、明暗主题检查点统一登记在 [docs/VISUAL_VERIFICATION.md](VISUAL_VERIFICATION.md) 的 UI-01 至 UI-30。视觉验证仍按需执行，不能接入 CI 或 Git Hook。
