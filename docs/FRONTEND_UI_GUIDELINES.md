...
管理员侧边栏按“业务运营”（用户管理、套餐管理、卡密管理）、“网络与节点”（节点管理、线路管理、证书管理、订阅模板、资源管理）、“监控与系统”（流量统计、系统日志、系统设置）三大结构化子分组呈现。订阅履约操作属于用户管理的综合弹窗；旧地址 `/admin/subscriptions` 仅作为兼容入口重定向至 `/admin/users`，不得再次作为平级菜单展示。
...
| 系统日志 | `/admin/logs` | 4 大指标 KPI 卡片、24h 错误/级别趋势堆叠柱状图、多维过滤器工具栏、高密度等宽日志表格、Live Tail 实时推流条、详情抽屉与清理确认弹窗 | 支持时间范围、级别、来源、节点、关键词与 TraceId 筛选；Live Tail 悬浮控制条支持暂停/恢复、清屏与滚动跟随；单条日志详情抽屉查看 TraceId 穿透、异常调用栈与 JSON 元数据；清理历史日志使用 AlertDialog 二次确认 |
