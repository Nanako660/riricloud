...
5. **操作卡片层级统一**：表单中承载 `Switch`、`Checkbox` 或操作按钮的卡片必须使用 shadcn/ui `FormItem` / `Card` 结构；与同层 `Input`、`SelectTrigger` 等基础控件并列时，外层卡片统一使用 `shadow-sm`，并沿用相同的高度、边框、圆角和内边距节奏。禁止用裸 `div` 另起一套相似卡片样式。
...
管理员侧边栏固定为 5 项：**用户管理、节点管理、系统设置、套餐管理、订阅模板**。订阅履约操作属于用户管理的综合弹窗；旧地址 `/admin/subscriptions` 仅作为兼容入口重定向至 `/admin/users`，不得再次作为平级菜单展示。
