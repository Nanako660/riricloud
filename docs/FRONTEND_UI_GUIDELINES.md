...
管理员侧边栏按“业务运营”（用户管理、套餐管理、卡密管理）、“网络与节点”（节点管理、线路管理、证书管理、订阅模板、资源管理）、“监控与系统”（流量统计、系统设置）三大结构化子分组呈现。订阅履约操作属于用户管理的综合弹窗；旧地址 `/admin/subscriptions` 仅作为兼容入口重定向至 `/admin/users`，不得再次作为平级菜单展示。
