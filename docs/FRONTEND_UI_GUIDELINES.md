...
| 卡密管理 | `/admin/redeem-codes` | 四态统计卡片、防抖搜索、状态筛选、卡密明文/掩码切换、CSV/TXT 导出、清理过期、行选择批量作废、兑换人与兑换时间列、有效期与未使用卡密作废 | 面额以元输入与展示，服务端按分保存；列表默认掩码显示卡密（保留前缀段与末 4 位），可用列头眼睛图标切换明文；批量生成结果在弹窗中集中复制；作废（单张/批量）与清理过期均使用 `AlertDialog` 二次确认 |
...
- 卡密管理列表使用表格容器内横向滚动与服务端分页（`Pagination` 组件），搜索使用 400ms 防抖；卡密等值现金凭据，列表默认掩码显示并提供明文切换；批量生成弹窗使用 React Hook Form + Zod，生成结果使用等宽文本和 `CopyButton` 换行复制；未使用卡密作废（单张与行选择批量）与清理过期必须经 `AlertDialog` 确认；导出 CSV/TXT 复用 blob 附件下载模式并携带当前筛选条件。
