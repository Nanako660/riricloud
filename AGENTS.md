...
2. **版本最小递增与 Unreleased 缓冲机制**：能 PATCH 不 MINOR，能 MINOR 不 MAJOR；三应用共用根 `package.json` 的统一版本号，不私设版本；日常特性与修复 PR 在 `CHANGELOG.md` 顶部的 `[Unreleased]` 缓冲区维护更新条目，版本号保持不变；发版时通过 Release PR 统一执行 `pnpm bump` 固化版本小节并同步发版（[docs/VERSIONING.md](docs/VERSIONING.md)）。
...
| 用户可感知的功能/修复/核心代码 | `CHANGELOG.md` 的 `[Unreleased]` 缓冲区（发版时通过 `pnpm bump` 固化） |
...
# 版本自增与版本门禁
pnpm bump                  # 固化 [Unreleased] 为 PATCH 版本（如 0.4.13 → 0.4.14）并重置空 Unreleased 模板
pnpm bump minor            # 固化 [Unreleased] 为 MINOR 版本（如 0.4.13 → 0.5.0）
pnpm bump major            # 固化 [Unreleased] 为 MAJOR 版本（如 0.4.13 → 1.0.0）
