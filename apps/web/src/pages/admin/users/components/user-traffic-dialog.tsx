...
export function UserTrafficDialog({ user, open, onOpenChange }: { user: AdminUser | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const { data, isPending, isError } = useUserTrafficDetail(user?.id ?? null, range, open);
  ...
  return (
    <ResponsiveDialog open={open} onOpenChange={onOpenChange}>
      <ResponsiveDialogContent size="wide" className="max-h-[85vh] overflow-y-auto">
        <div className="space-y-5">
          ...
          {isPending ? <div className="space-y-4"><Skeleton className="h-32" /><div className="grid gap-4 md:grid-cols-2"><Skeleton className="h-64" /><Skeleton className="h-64" /></div></div> : isError || !data ? <EmptyState title="无法加载用户流量" description="请稍后刷新重试" /> : (
            <>
              ...
              <Card className="min-w-0"><CardHeader><CardTitle className="text-base">用户线路使用清单</CardTitle></CardHeader><CardContent className="p-0"><TrafficRankTable items={data.lineBreakdown} compact /></CardContent></Card>
            </>
          )}
        </div>
