...
const nodeDetailSchema = z.object({
  ...
  serverHost: z.string().trim().min(1, '请输入服务器地址'),
...
export default function NodeDetailPage() {
  ...
  const { data: binaryResources } = useAdminBinaryResources();
  ...
  const form = useForm<NodeDetailFormValues>({
    ...
    defaultValues: { name: '', serverHost: '', configOverride: '' }
  });
  ...
  useFormResetOnKey({
    ...
    reset: () => form.reset({ name: node?.name ?? '', serverHost: node?.serverHost ?? '', configOverride: node?.configOverride ?? '' })
  });
  ...
  const saveBasic = async () => {
    if (!(await form.trigger(['name', 'serverHost']))) return;
    const values = form.getValues();
    updateNode.mutate({ id: node.id, name: values.name, serverHost: values.serverHost });
  };
  const saveOverride = () => {
