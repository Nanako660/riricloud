...
function InstallCommandDialog({ open, onOpenChange, node }: { open: boolean; onOpenChange: (open: boolean) => void; node: AdminNode }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent size="compact"><DialogHeader><DialogTitle>Agent 原生 CLI</DialogTitle><DialogDescription>在目标 VPS 上以 root 身份执行安装命令；安装后使用 Agent 自带的生命周期命令运维。</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label>WS / WSS 长连接</Label><div className="flex items-start gap-2"><code className="min-w-0 flex-1 break-all rounded-md border bg-muted/40 p-3 text-xs">{commands.ws}</code><CopyButton value={commands.ws} /></div></div><div className="space-y-2"><Label>HTTP / HTTPS 轮询</Label><div className="flex items-start gap-2"><code className="min-w-0 flex-1 break-all rounded-md border bg-muted/40 p-3 text-xs">{commands.http}</code><CopyButton value={commands.http} /></div></div><div className="space-y-2"><Label>彻底卸载</Label><div className="flex items-start gap-2"><code className="min-w-0 flex-1 break-all rounded-md border bg-muted/40 p-3 text-xs">{uninstallCommand}</code><CopyButton value={uninstallCommand} /></div></div></div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>关闭</Button></DialogFooter></DialogContent></Dialog>;
}
