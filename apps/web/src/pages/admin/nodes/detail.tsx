...
function InstallCommandDialog({ open, onOpenChange, node }: { open: boolean; onOpenChange: (open: boolean) => void; node: AdminNode }) {
  const commands = node.installCommands ?? { ws: `curl -fsSL https://<master-domain>/api/v1/install.sh | bash -s -- --token=${node.agentToken} --master=wss://<master-domain>/ws/agent`, http: `curl -fsSL https://<master-domain>/api/v1/install.sh | bash -s -- --token=${node.agentToken} --master=http://<master-domain>` };
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent size="compact"><DialogHeader><DialogTitle>Agent 安装命令</DialogTitle><DialogDescription>选择节点网络可用的通信模式，在目标 VPS 上以 root 身份执行。</DialogDescription></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label>WS / WSS 长连接</Label><div className="flex items-start gap-2"><code className="min-w-0 flex-1 break-all rounded-md border bg-muted/40 p-3 text-xs">{commands.ws}</code><CopyButton value={commands.ws} /></div></div><div className="space-y-2"><Label>HTTP / HTTPS 轮询</Label><div className="flex items-start gap-2"><code className="min-w-0 flex-1 break-all rounded-md border bg-muted/40 p-3 text-xs">{commands.http}</code><CopyButton value={commands.http} /></div></div></div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>关闭</Button></DialogFooter></DialogContent></Dialog>;
}
