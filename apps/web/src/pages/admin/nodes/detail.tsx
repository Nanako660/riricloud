...
import { ArrowLeft, Cpu, GitBranch, KeyRound, Network, RefreshCw, Trash2 } from 'lucide-react';
...
export default function NodeDetailPage() {
  ...
  const { updateNode, deleteNode, reloadNode, upgradeNode, probeNode, waitForTask } = useNodeMutations();
  ...
  return <PageContainer>
     <div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><Button variant="ghost" size="icon" asChild aria-label="返回"><Link to="/admin/nodes"><ArrowLeft className="h-4 w-4" /></Link></Button><div className="min-w-0"><h1 className="truncate text-2xl font-semibold tracking-tight">{node.name}</h1><p className="truncate text-sm text-muted-foreground">{node.serverHost}</p></div><Badge variant={node.status === 'ONLINE' && node.communicationMode === 'WS' ? 'default' : 'secondary'}>{statusLabel}</Badge></div><div className="flex gap-2"><Button variant="outline" size="sm" disabled={reloadNode.isPending} onClick={() => reloadNode.mutate(node.id)}><RefreshCw />重载</Button><Button variant="outline" size="sm" onClick={() => setProbeOpen(true)}><Network />探针</Button><Button variant="outline" size="sm" onClick={() => setUpgradeOpen(true)}>升级</Button></div></div>
    ...
    <Tabs defaultValue="lines"><TabsList><TabsTrigger value="lines">线路承载</TabsTrigger><TabsTrigger value="basic">基础与遥测</TabsTrigger><TabsTrigger value="advanced">高级与运维</TabsTrigger></TabsList>
      ...
      <TabsContent value="basic" className="space-y-4"><Card><CardHeader><CardTitle className="text-base">基础信息</CardTitle></CardHeader><CardContent className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="node-name">节点名称</Label><Input id="node-name" value={name} onChange={(event) => setName(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="node-host">服务器地址</Label><Input id="node-host" value={serverHost} onChange={(event) => setServerHost(event.target.value)} /></div></div><Button size="sm" disabled={updateNode.isPending} onClick={saveBasic}>保存基础信息</Button></CardContent></Card><Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><KeyRound className="h-4 w-4" />Agent 接入</CardTitle></CardHeader><CardContent className="space-y-3"><div className="flex items-center gap-2"><code className="min-w-0 flex-1 truncate rounded-md border bg-muted/50 px-3 py-2 text-xs">{node.agentToken}</code><CopyButton value={node.agentToken} /></div><div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground"><span>通信模式：{node.communicationMode === 'HTTP' ? 'HTTP / HTTPS 轮询' : 'WS / WSS 长连接'}</span><span>轮询建议：{node.pollIntervalSecs} 秒</span><span>最近上报：{node.lastSeenAt ? new Date(node.lastSeenAt).toLocaleString('zh-CN') : '未上报'}</span></div><p className="text-sm text-muted-foreground">内核状态：{node.kernelRunning == null ? '未知' : node.kernelRunning ? '运行中' : '未运行'}</p></CardContent></Card><Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Cpu className="h-4 w-4" />实时遥测</CardTitle></CardHeader><CardContent className="grid grid-cols-3 gap-4 text-sm text-muted-foreground"><span>CPU：{node.cpuUsage != null ? `${node.cpuUsage.toFixed(1)}%` : '—'}</span><span>内存：{node.memoryUsage != null ? `${node.memoryUsage.toFixed(1)}%` : '—'}</span><span>带宽：{node.bandwidthRate != null ? `${(node.bandwidthRate / 1024).toFixed(1)} KB/s` : '—'}</span></CardContent></Card></TabsContent>
    ...
    </Tabs><UpgradeNodeDialog open={upgradeOpen} onOpenChange={setUpgradeOpen} pending={upgradeNode.isPending} onSubmit={(values) => upgradeNode.mutate({ id: node.id, ...values }, { onSuccess: (data: { taskId: string; requested: boolean }) => { setUpgradeOpen(false); if (node.communicationMode === 'HTTP' && data.requested) void waitForTask({ nodeId: node.id, taskId: data.taskId, label: '升级任务' }); } })} /><ProbeNodeDialog open={probeOpen} onOpenChange={setProbeOpen} pending={probeNode.isPending} onSubmit={(values) => probeNode.mutate({ id: node.id, ...values }, { onSuccess: (data: { taskId: string; requested: boolean }) => { setProbeOpen(false); if (node.communicationMode === 'HTTP' && data.requested) void waitForTask({ nodeId: node.id, taskId: data.taskId, label: '探针任务' }); } })} />
  </PageContainer>;
}
