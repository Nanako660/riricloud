...
function GeneratedConfigPreview({ node }: { node: { id: string; lines: NodeLine[] } }) {
  const inbounds = node.lines.flatMap((line) => {
    ...
    if (line.exitNodeId === node.id && line.type === 'RELAY' && line.relayMode !== 'TARGET_LINE') result.push({ type: line.protocolType.toLowerCase(), tag: `line-${line.id}-exit`, listen: '0.0.0.0', listen_port: line.exitPort });
    return result;
  });
