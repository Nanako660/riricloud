...
function MirrorForm({ open, editing, nodes, pending, onOpenChange, onSubmit }: { open: boolean; editing: ApiMirror | null; nodes: ReturnType<typeof useAdminNodes>['data']; pending: boolean; onOpenChange: (open: boolean) => void; onSubmit: (payload: MirrorPayload) => void }) {
  ...
  const valid = name.trim() && /^[a-z0-9][a-z0-9-]{2,62}$/.test(slug.trim()) && /^https?:\/\//i.test(upstream.trim()) && nodeId && origins.trim();
  const submit = (event: React.FormEvent) => { event.preventDefault(); if (!valid) return; onSubmit({ name: name.trim(), slug: slug.trim().toLowerCase(), upstreamBaseUrl: upstream.trim(), allowedOrigins: origins.split(/[\n,]/).map((item) => item.trim()).filter(Boolean), nodeId, accessMode, enabled, ...(accessMode === 'SHARE' && expires ? { shareExpiresAt: new Date(expires).toISOString() } : {}) }); };
  return <ResponsiveDialog open={open} onOpenChange={onOpenChange}><ResponsiveDialogContent size="wide"><DialogHeader><DialogTitle>{editing ? '编辑镜像站' : '新增镜像站'}</DialogTitle><DialogDescription>固定一个公开上游，并指定具备 WS 镜像能力的出网节点。</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={submit}><div className="grid gap-3 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="mirror-name">名称</Label><Input id="mirror-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="GitHub Raw" /></div><div className="space-y-2"><Label htmlFor="mirror-slug">Slug</Label><Input id="mirror-slug" value={slug} onChange={(event) => setSlug(event.target.value)} placeholder="github-raw" /></div></div><div className="space-y-2"><Label htmlFor="mirror-upstream">上游基址</Label><Input id="mirror-upstream" type="url" value={upstream} onChange={(event) => setUpstream(event.target.value)} placeholder="https://raw.githubusercontent.com" /></div><div className="space-y-2"><Label htmlFor="mirror-origins">允许的上游域名</Label><Textarea id="mirror-origins" value={origins} onChange={(event) => setOrigins(event.target.value)} placeholder="raw.githubusercontent.com\nobjects.githubusercontent.com" rows={3} /><p className="text-xs text-muted-foreground">每行一个域名，重定向只能落在这些域名内。</p></div><div className="grid gap-3 sm:grid-cols-2"><div className="space-y-2"><Label>出网节点</Label><Select value={nodeId} onValueChange={setNodeId}><SelectTrigger><SelectValue placeholder="选择在线 WS 节点" /></SelectTrigger><SelectContent>{availableNodes.map((node) => <SelectItem key={node.id} value={node.id}>{node.name} · {node.serverHost}</SelectItem>)}</SelectContent></Select>{!availableNodes.length && <p className="text-xs text-destructive">暂无可用节点，请先升级支持镜像代理的 Agent。</p>}</div><div className="space-y-2"><Label>访问策略</Label><Select value={accessMode} onValueChange={(value) => setAccessMode(value as MirrorAccessMode)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ADMIN">管理员会话</SelectItem><SelectItem value="SHARE">分享 Token</SelectItem><SelectItem value="PUBLIC">公开访问</SelectItem></SelectContent></Select></div></div>{accessMode === 'SHARE' && <div className="space-y-2"><Label htmlFor="mirror-expires">分享有效期（可选）</Label><Input id="mirror-expires" type="datetime-local" value={expires} onChange={(event) => setExpires(event.target.value)} /></div>}<div className="flex items-center justify-between rounded-md border p-3"><div><Label htmlFor="mirror-enabled">启用镜像站</Label><p className="text-xs text-muted-foreground">首次上线建议先使用管理员或分享模式验证。</p></div><Switch id="mirror-enabled" checked={enabled} onCheckedChange={setEnabled} /></div><DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>取消</Button><Button type="submit" disabled={!valid || !availableNodes.length || pending}>{pending ? '保存中…' : editing ? '保存镜像站' : '创建镜像站'}</Button></DialogFooter></form></ResponsiveDialogContent></ResponsiveDialog>;
}
