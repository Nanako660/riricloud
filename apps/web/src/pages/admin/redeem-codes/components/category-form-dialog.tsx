...
export function CategoryFormDialog({ open, onOpenChange, category, plans, pending, onSubmit }: CategoryFormDialogProps) {
  ...
  const schema = React.useMemo(() => z.object({
    ...
    rewardAmountYuan: z.coerce.number().positive().multipleOf(0.01).optional(),
    planId: z.string(),
    limit: z.string().refine((value) => value === '' || (/^[1-9]\d*$/.test(value) && Number.isSafeInteger(Number(value))))
  }).superRefine((value, ctx) => {
    if (value.rewardType === 'BALANCE' && (!value.rewardAmountYuan || value.rewardAmountYuan <= 0)) ctx.addIssue({ code: 'custom', path: ['rewardAmountYuan'], message: t('admin:redeemCodes.valAmountPositive') });
  ...
  return <ResponsiveDialog open={open} onOpenChange={onOpenChange}>
    <ResponsiveDialogContent>
      ...
      <Form {...form}><form onSubmit={form.handleSubmit(submit)} className="grid gap-4 sm:grid-cols-2">
        ...
        <FormField control={form.control} name="limit" render={({ field }) => <FormItem className="sm:col-span-2"><FormLabel>{t('admin:redeemCodes.identityLimit')}</FormLabel><FormControl><Input inputMode="numeric" placeholder="1" {...field} /></FormControl><FormDescription>{t('admin:redeemCodes.identityLimitDesc')}</FormDescription><FormMessage /></FormItem>} />
