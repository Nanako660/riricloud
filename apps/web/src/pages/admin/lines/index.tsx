...
const relayLabels: Record<RelayMode, string> = { BLIND_FORWARD: '盲转发', PROTOCOL_PROXY: '协议代理', TARGET_LINE: '桥接已有线路' };
      ...
      <div className="flex min-w-0 flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex w-full flex-col gap-2 sm:flex-row lg:w-auto">
          ...
          >
            ...
            {speedtestAll.isPending ? '全量测速中…' : '全部测速'}
          </Button>
          <Button className="w-full sm:w-auto" onClick={openCreate}><Plus />新建线路</Button>
        </div>
      </div>
        ...
        <CardContent className="p-0">
          {lines.length ? <Table className="min-w-[980px]">
            ...
            <TableBody>{lines.map((line, index) => <TableRow key={line.id}>
              ...
              <TableCell><div className="flex max-w-40 flex-wrap gap-1">{Boolean(line.speedLimitMbps) && <Badge variant="outline" className="gap-1 border-amber-500/40 text-amber-600 dark:text-amber-400"><Zap className="size-3" />{line.speedLimitMbps}M</Badge>}{line.tags.map((item) => <Badge key={item} variant="secondary">#{item}</Badge>)}<Badge variant="outline">{line.trafficRate}x</Badge></div></TableCell>
              <TableCell>
                <LineLatencyChip
