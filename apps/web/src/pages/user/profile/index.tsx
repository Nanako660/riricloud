    ...
    <PageContainer>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4 text-sm"><div><p className="text-muted-foreground">登录邮箱</p><p className="font-medium">{user.data.email}</p></div><div><p className="text-muted-foreground">账号角色</p><Badge variant={user.data.role === 'ADMIN' ? 'default' : 'secondary'}>{user.data.role === 'ADMIN' ? '管理员' : '普通用户'}</Badge></div><div className="space-y-2"><p className="text-muted-foreground">用户代理凭据</p><div className="flex min-w-0 gap-2"><code className="min-w-0 flex-1 truncate rounded-md border bg-muted/50 px-3 py-2 text-xs">{user.data.uuid}</code><CopyButton value={user.data.uuid} /></div><Button type="button" variant="outline" size="sm" onClick={() => setResetOpen(true)} disabled={resetUuid.isPending}>重置代理凭据</Button><p className="text-xs text-muted-foreground">重置后所有客户端需要重新导入订阅。</p></div></CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle>修改登录密码</CardTitle><CardDescription>修改后当前会话保持有效，下一次登录使用新密码。</CardDescription></CardHeader>
          <CardContent><Form {...passwordForm}><form onSubmit={passwordForm.handleSubmit(onPassword)} className="space-y-4">{(['oldPassword', 'newPassword', 'confirmPassword'] as const).map((name) => <FormField key={name} control={passwordForm.control} name={name} render={({ field }) => <FormItem><FormLabel>{name === 'oldPassword' ? '当前密码' : name === 'newPassword' ? '新密码' : '确认新密码'}</FormLabel><FormControl><Input type="password" autoComplete="new-password" {...field} /></FormControl><FormMessage /></FormItem>} />)}<Button type="submit" disabled={changePassword.isPending}>{changePassword.isPending ? '保存中…' : '保存新密码'}</Button></form></Form></CardContent>
        </Card>
      </div>
