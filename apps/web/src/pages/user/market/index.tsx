    ...
    <PageContainer>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {plans.data?.map((plan) => (
          <Card
            key={plan.id}
            className={current?.subscription?.plan.id === plan.id ? 'border-primary ring-1 ring-primary/20' : ''}
          >
            <CardHeader>
              <div className="flex items-start justify-between gap-3">
                <CardTitle className="text-lg">{plan.name}</CardTitle>
                {current?.subscription?.plan.id === plan.id && <Badge>当前套餐</Badge>}
              </div>
            ...
            <CardFooter>
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button
                    className="w-full"
                    variant={current?.subscription?.plan.id === plan.id ? 'outline' : 'default'}
                    onClick={() => setSelected(plan)}
                    disabled={current?.subscription?.plan.id === plan.id}
                  >
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      {active
                        ? '新套餐即时生效，已用流量会重置，周期按新套餐重新计算。'
                        : `将订购「${selected?.name}」，支付状态由管理员按当前部署流程处理。`}
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
