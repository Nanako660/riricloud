...
describe('BinariesService', () => {
  const prisma = { node: { findUnique: jest.fn(), findFirst: jest.fn() } };
  ...
  it('生成不携带 AgentToken 的主控内部下载地址', () => {
    delete process.env.RIRICLOUD_PUBLIC_URL;
    expect(service.buildDownloadUrl('agent-linux-amd64', 'secret-token')).toContain('/api/v1/downloads/binaries/agent-linux-amd64');
    expect(service.buildDownloadUrl('agent-linux-amd64', 'secret-token')).not.toContain('token=');
  });

  it('公网地址为空白时回退到主控本地地址', () => {
    process.env.RIRICLOUD_PUBLIC_URL = '  ';
    process.env.PORT = '33000';
    expect(service.buildDownloadUrl('agent-linux-amd64', 'secret-token')).toBe('http://localhost:33000/api/v1/downloads/binaries/agent-linux-amd64');
    delete process.env.RIRICLOUD_PUBLIC_URL;
  ...
  it('配置二进制分发基准地址时优先使用系统设置', async () => {
    ...
    await expect(configured.buildConfiguredDownloadUrl('agent-linux-amd64', 'secret-token')).resolves.toBe(
      'https://cdn.example.com/riricloud/api/v1/downloads/binaries/agent-linux-amd64'
    );
  });
  ...
  it('系统设置未配置时使用请求域名生成二进制地址', async () => {
    ...
    await expect(configured.buildConfiguredDownloadUrl('agent-linux-amd64', 'secret-token', 'https://panel.example.com')).resolves.toBe(
      'https://panel.example.com/api/v1/downloads/binaries/agent-linux-amd64'
    );
  });
