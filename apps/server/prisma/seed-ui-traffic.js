...
async function main() {
  ...
  const lines = {
    hkVless: await upsertLine('UI 香港 Premium · VLESS', { tag: 'ui-hk-vless', protocolType: 'VLESS', params: { flow: 'xtls-rprx-vision', transport: { type: 'tcp' }, tls: { enabled: true, mode: 'reality', serverName: 'www.apple.com' } }, entryNodeId: nodes.hongKong.id, entryPort: 24101, exitNodeId: nodes.hongKong.id, exitPort: 24101, serverHost: nodes.hongKong.serverHost, trafficRate: lineRates[0], tags: ['hk', 'premium'], sortOrder: 20 }),
    hkShadowsocks: await upsertLine('UI 香港低倍率 · Shadowsocks', { tag: 'ui-hk-ss', protocolType: 'SHADOWSOCKS', params: { method: '2022-blake3-aes-128-gcm', password: 'ui-demo-password', mode: 'shared' }, entryNodeId: nodes.hongKong.id, entryPort: 24102, exitNodeId: nodes.hongKong.id, exitPort: 24102, serverHost: nodes.hongKong.serverHost, trafficRate: lineRates[1], tags: ['hk', 'economy'], sortOrder: 21 }),
    tokyoRelay: await upsertLine('UI 东京中继 · Hysteria2', { tag: 'ui-tokyo-relay', type: 'RELAY', relayMode: 'BLIND_FORWARD', protocolType: 'HYSTERIA2', params: { upMbps: 100, downMbps: 500, tls: { enabled: true, mode: 'tls', serverName: 'ui-tokyo.example.test', insecure: true } }, entryNodeId: nodes.tokyo.id, entryPort: 24111, exitNodeId: nodes.losAngeles.id, exitPort: 24121, serverHost: nodes.tokyo.serverHost, trafficRate: lineRates[2], tags: ['premium', 'relay', 'us'], sortOrder: 22 }),
    tokyoTuic: await upsertLine('UI 东京直连 · TUIC', { tag: 'ui-tokyo-tuic', protocolType: 'TUIC', params: { congestionControl: 'bbr', zeroRttHandshake: false, heartbeat: '10s', tls: { enabled: true, mode: 'tls', serverName: 'ui-tokyo.example.test', insecure: true } }, entryNodeId: nodes.tokyo.id, entryPort: 24112, exitNodeId: nodes.tokyo.id, exitPort: 24112, serverHost: nodes.tokyo.serverHost, trafficRate: lineRates[3], tags: ['jp', 'premium'], sortOrder: 23 }),
    laVless: await upsertLine('UI 洛杉矶 · VLESS Reality', { tag: 'ui-la-vless', protocolType: 'VLESS', params: { flow: 'xtls-rprx-vision', transport: { type: 'tcp' }, tls: { enabled: true, mode: 'reality', serverName: 'www.microsoft.com' } }, entryNodeId: nodes.losAngeles.id, entryPort: 24131, exitNodeId: nodes.losAngeles.id, exitPort: 24131, serverHost: nodes.losAngeles.serverHost, trafficRate: lineRates[4], tags: ['us', 'premium'], sortOrder: 24 }),
    laTrojan: await upsertLine('UI 洛杉矶备用 · Trojan', { tag: 'ui-la-trojan', protocolType: 'TROJAN', params: { transport: { type: 'ws', path: '/ui-demo' }, tls: { enabled: true, mode: 'tls', serverName: 'ui-la.example.test', insecure: true } }, entryNodeId: nodes.losAngeles.id, entryPort: 24132, exitNodeId: nodes.losAngeles.id, exitPort: 24132, serverHost: nodes.losAngeles.serverHost, trafficRate: lineRates[5], tags: ['us', 'backup'], sortOrder: 25 }),
    disabled: await upsertLine('UI 香港 · 已停用线路', { tag: 'ui-hk-disabled', protocolType: 'VLESS', params: { transport: { type: 'tcp' }, tls: { enabled: false, mode: 'none' } }, entryNodeId: nodes.hongKong.id, entryPort: 24103, exitNodeId: nodes.hongKong.id, exitPort: 24103, serverHost: nodes.hongKong.serverHost, trafficRate: 1, tags: ['hk', 'maintenance'], status: 'DISABLED', isPublic: false, sortOrder: 26 })
  };
